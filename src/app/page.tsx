...
export default function HomePage() {
  ...
  const handleTransfer = async () => {
    ...
    const amountBigInt = parseEther(transferAmount); // assuming 18 decimals; adjust if not
    try {
      await writeContract({
        address: tokenAddress,
        abi: tokenAbi,
        ...
        <div className="w-full max-w-3xl space-y-6">
          ...
          <section className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 space-y-3">
            ...
            <p className="text-sm text-slate-400">
              ...
              <span className="text-slate-100">{tokenSymbol ?? 'VIRD'}</span>)
            </p>
            <p className="text-sm text-slate-400">
              Decimals:{' '}
              <span className="text-slate-100">{tokenDecimals?.toString() ?? '...'}</span>
            </p>
            <p className="text-sm text-slate-400">
              Your balance:{' '}
